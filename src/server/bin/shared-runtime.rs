...
lazy_static! {
    static ref RUNTIME: StdRwLock<Option<Runtime>> =
        StdRwLock::new(Some(Runtime::new().expect("Error creating tokio runtime.")));
    static ref HANDLE: Arc<Tokio1Handle> =
        Arc::new(RUNTIME.read().unwrap().as_ref().unwrap().handle().clone());
}
...
fn main() {
    ...
    let tokio_1_handle = (**HANDLE).clone();
    ...
    let c_tokio_1_handle = (**HANDLE).clone();
    let c_amqp_client = amqp_client.clone();
    let sub_ft = async move {
        ...
        while let Some(item) = subscriber.next().await {
            ...
            let cc_tokio_1_handle = c_tokio_1_handle.clone();
            c_tokio_1_handle.spawn(async move {
                let start_time = Instant::now();
                let mock_telemetry_task = async move {
                    let duration = start_time.elapsed();
                    println!("{:?}, {:?}", duration.as_nanos(), duration);
                };
                let _ = cc_tokio_1_handle.spawn(mock_telemetry_task);
            });
